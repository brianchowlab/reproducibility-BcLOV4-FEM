function [mesh_c,poly,shp_n] = GenMesh(contours,param)
    ...
    if strcmp(param.mesh,'NA')
       ...
       mesh_t = triangulation(mesh_c.Elements',mesh_c.Nodes');
       bound = freeBoundary(mesh_t);
       mesh_t = triangulation(bound,mesh_c.Nodes');
       trimesh(mesh_t,'facealpha',0.25);
       hold on
       
        %Lysosomes
        r = .34;
        [x1,y1,z1] = sphere(24);
        xyz_min = min(mesh_c.Nodes');
        xyz_max = max(mesh_c.Nodes');
        lyso_counter = 0;
        center = [];
        elements = [mesh_t.ConnectivityList'];
        nodes = [mesh_t.Points'];
        while lyso_counter < 300
            x0 = rand*(xyz_max(1) - xyz_min(1)) + xyz_min(1);
            y0 = rand*(xyz_max(2) - xyz_min(2)) + xyz_min(2);
            z0 = rand*(xyz_max(3) - xyz_min(3)) + xyz_min(3);
            

            
            [~,d] = nearestNeighbor(mesh_t,[x0,y0,z0]);
           ...
           if any(~inpolyhedron(bound,mesh_c.Nodes',[x2,y2,z2]))
                continue
            end
            
            if lyso_counter > 1
               dists = sqrt(sum((center - [x0,y0,z0]).^2,2));
               if min(dists) < 2*r + 0.34
                  disp('Skipping')
                  continue
               end
            end
            ...
            P = [x2,y2,z2];
            P = unique(P,'rows');
            shp_l = alphaShape(P(:,1),P(:,2),P(:,3),1);
            %plot(shp_l)
            
            [elements_l,nodes_l] = boundaryFacets(shp_l);
            elements_l = fliplr(elements_l);

            elements = [elements';elements_l+size(nodes,2)]';
            nodes = [nodes';nodes_l]';
         
            lyso_counter = lyso_counter + 1
        end
        
        model = createpde();
        gm_c = geometryFromMesh(model,nodes,elements);
        %mesh_c = generateMesh(model,'GeometricOrder','linear','Hmin',param.min_element_size,'Hmax',param.max_element_size);%mesh_c = generateMesh(model,'Hmin',0.5,'Hmax',30,'GeometricOrder','linear');
        mesh_c = generateMesh(model,'Hmin',0.05,'Hmax',25,'GeometricOrder','linear');
    end
    if param.plot
        figure
