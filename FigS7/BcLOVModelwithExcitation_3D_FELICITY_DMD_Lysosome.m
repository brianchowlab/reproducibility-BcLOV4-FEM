...
while flag == 0
    try
        [mesh_c,poly,shp_no] = GenMesh_Lysosome(contours,param);
        %[mesh_c,poly,shp_n] = GenMesh(contours,param);
        pts = shp_no.Points;
        pts(:,1:2) = shp_no.Points(:,1:2) * param.scale_len;
        pts(:,3) = shp_no.Points(:,3) * param.z_resolution;
        shp_n = alphaShape(pts,shp_no.Alpha*param.scale_len*2);
...
props = MeshProps_Lysosomes_Exp(Mesh,shp_no);
...
%param.conc = 1.364;
%param.conc = (967.68-param.offset)/param.conc_ratio;
...
save([filename,'-nl-3D-lyso-300.mat'],'Soln','sol_M','I','contours','param','props','tlist','desired_times','pdem_C','gm_C_f');
...
param.axial_resolution = 0.2167;

c_intrp = InterpolateCytoplasm(pde_C,1:length(desired_times(1:param.interpolation_interval:end)),I,param);

m_intrp = InterpolateMembrane_Lysosomes(I,1:param.interpolation_interval:length(desired_times),props.pm_TR,sol_M,param);
...
%s = 30
for i = 1:size(c_intrp,4)
	imwrite(uint16(squeeze(c_intrp(:,:,s,i))),['./',folder_name_no_PSF,'/',num2str(i),'.tif']);
end
...
%% Convolve with PSF
param.PSF = '/home/ivan/Documents/BcLOV4-FEM/psf_60x_1.4_oil.tif';
...
s = 30
for i = 1:size(c_intrp,4)
	imwrite(uint16(squeeze(c_intrp_blurred(:,:,s,i))),['./',folder_name_PSF,'/',num2str(i),'.tif']);
end
